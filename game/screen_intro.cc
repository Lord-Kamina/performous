...
void ScreenIntro::draw_menu_options() {
	...
	float submenuanim = 1.0f - std::min(1.0, std::abs(m_submenuAnim.get()-m_menu.getSubmenuLevel()));
	...
	for (size_t i = start_i, ii = 0; ii < showopts && i < opts.size(); ++i, ++ii) {
		...
		if (i == m_menu.curIndex()) {
			...
			wcounter = std::max(static_cast<double>(wcounter), theme->option_selected.w() + 2 * sel_margin); // Calculate the widest entry
		...
		// Regular option (not selected)
			...
			txt.draw(title);
			wcounter = std::max(static_cast<double>(wcounter), txt.w() + 2 * sel_margin); // Calculate the widest entry
		}
	}
